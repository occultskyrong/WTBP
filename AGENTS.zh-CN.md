...
3. 执行 `make commit-checklist`。它会刷新默认分支，并在不修改工作区的前提下检查可合并性，以及 `VERSION`/`CHANGELOG.md` 是否成对变更；任一检查失败都必须停止，
   不得创建提交或推送。
4. 使用已校验的范围创建提交，提交标题使用中文说明的 Conventional Commit 格式。
5. 将当前任务分支推送到已配置的远端；不得强推或改写历史。`pre-push` Hook 会在刷新默认分支后再次执行可合并性预检。
6. 推送成功后执行 `make return-to-default`。仅当任务工作树干净、HEAD 已完整推送，且默认分支未被其他工作树占用时，
   它才切换并同步到最新默认分支；否则只报告安全跳过。该步骤用于分支卫生，不能替代可合并性预检。下一个独立任务必须从
   干净且已同步的默认分支工作树开始。
7. 核验远端分支和分支归位结果，分别报告提交和推送结果。创建 PR 不属于此流程，只有用户另行明确要求时才执行。版本变更后的内容
