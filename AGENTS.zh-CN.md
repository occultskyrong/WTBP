...
1. 检查工作区、暂存范围、当前分支、远端分歧和拟使用的 Conventional Commit 提交消息。
2. 执行 `make commit-checklist`。任一检查失败都必须停止，不得创建提交、推送或 PR。
3. 使用已校验的范围创建提交，提交标题使用中文说明的 Conventional Commit 格式。
4. 将当前分支推送到已配置的远端；不得强推或改写历史。
5. 向仓库配置的默认/目标分支创建 PR，标题和正文使用中文，说明变更、受影响的 Skill 或 Practice、验证结果、
   dry-run 边界和未解决风险。
6. 核验远端分支和 PR 的 URL/状态，分别报告提交、推送和 PR 结果。
