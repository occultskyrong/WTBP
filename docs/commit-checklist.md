...
当前版本记录在根目录 [`VERSION`](../VERSION)，格式固定为 `MAJOR.MINOR.PATCH`；每次版本变动必须同步更新
[`CHANGELOG.md`](../CHANGELOG.md)。版本只代表 WTBP 规范和可复用内容的对外认知，不代表每个 Git 提交都必须增加数字。
版本号只能在存在祖先关系的一条版本线中递增：`master` 是已合并主线，只有其 Tag 才是正式发布；未合并任务分支的版本仅是待合并记录，不能直接作为
另一条独立任务分支的基线。完整规则见 [`docs/versioning.md`](versioning.md)。
...
合并最新默认分支时，版本以该默认分支的 `VERSION` 为基线：任务分支仍包含独立规范变更时，继续递增该基线；不得保留更低版本或回退版本号。任务若依赖未合并分支，必须先合并其 PR，或在明确授权下从该分支新建任务分支并声明依赖；不得只复制其版本号。
...
- 变更 `VERSION` 时，`CHANGELOG.md` 已同步新增当前版本条目，并明确该条目属于 `master` 主线还是未合并任务线。
