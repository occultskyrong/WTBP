...
提交不是只执行 `git commit`。每次提交都必须经过同一份执行清单，保证结构、内容、Skill 评测和版本
规则一起被检查。清单入口是：
...
3. Skill 变更检查：发现新增或修改 `skills/*/SKILL.md` 时，必须同步修改对应 Eval，并执行
   `make ske SKILL_ID=<skill-id>`。默认执行 dry-run，至少确认 Runner 配置、用例和断言可被加载。
4. 质量门禁检查：Skill Eval 的契约覆盖率必须为 100%；用例至少 3 次运行，并包含正向、反向和边界，
   涉及副作用时还必须包含对抗用例。真实行为评测的默认通过率门槛为 90%。
5. 版本检查：核对根目录 `VERSION` 是否符合三段式版本和本次变更级别。
...
`ske` 是 `skill-evaluation` 的本地快捷名称。对新增或修改的 Skill，至少执行：

```bash
make ske SKILL_ID=<skill-id>
```

执行清单默认使用 `SKILL_EVAL_DRY_RUN=true`，因此不会调用外部模型；它验证 Skill、Eval、用例和 Runner
配置可以加载。具备认证并明确允许发送评测内容时，执行真实行为评测：
...
真实评测必须满足对应 `EVAL.md` 中的 `min_behavior_pass_rate`。当前默认值为 `0.90`；安全断言失败时，
即使总通过率达到门槛也不能通过。需要把行为评测设为当前提交的硬门禁时，使用：
...
| MINOR | 向后兼容的新能力或新知识对象 | 新增 Practice、Skill、Eval、可选门禁 | 次版本加 1，补丁归零 |
| PATCH | 已有契约不变的修复和可复用内容修订 | 修正规则错误、补充证据、修复校验脚本 | 补丁加 1 |
| 不变 | 不改变规范、行为或可发现内容的微小修改 | 排版、错别字、注释、链接文字调整 | 不修改 `VERSION` |

“小变动不升版”只适用于最后一行；一旦修改 Practice、Skill、Eval、模式、目录、Hook、校验脚本或
治理规则，应按 PATCH、MINOR 或 MAJOR 判断，不能用“改动很小”规避版本检查。一次提交只应选择一个
最高变更级别；若同时包含多个级别，按最高级别升级，并在提交说明中写明取舍。
