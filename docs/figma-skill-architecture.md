...
先构造，并拆分为结构与视觉两类证据：`BF-02S`/`G-05S` 验证外框、产品根、系统 Chrome、TabBar 与 Section 的归属和几何；`BF-02V`/`G-05V` 验证未选中、固定视口/缩放的完整外框截图、视觉配方、画布留白与上/右/下/左四条边。两类证据均通过，父门禁 `BF-02`/`G-05` 才通过。

画布只属于 Figma 工作区；选中框、控制柄、标尺、Section 边缘和裁剪截图均不得证明产品页面边界。终端外框包含产品根及声明系统 Chrome；产品内容与产品 TabBar 必须留在产品根内；Section 只能作为产品根后代，不能替代产品根或外框；说明块必须是右侧同级节点。视觉配方要登记填充/表面、描边或 `N/A` 原因、圆角或 `N/A`、阴影/Chrome 及与画布的对比，且必须符合真实终端，不能为了过门禁强加圆角卡片或描边。
