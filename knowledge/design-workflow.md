...
The gate record must include the artifact revision, inspected page/frame IDs, target shell contract, base-frame IDs and `BF-01`–`BF-06` results, `G-01`–`G-06` results, geometry sources, screenshots, exceptions, and the rerun result. A node-write success, static check, or single screenshot cannot satisfy this gate.
