...
- 结构、动作组或 Icon 相关变更必须在页面样式前先创建并审批 `INV-YYYYMMDD-VNN`，再建立或确认 `AGC-YYYYMMDD-VNN` 并记录 `AGC-01`–`AGC-06`，之后建立或确认 `ICON-YYYYMMDD-VNN` 并记录 `IA-01`–`IA-07`；小范围 patch 只有在批准范围、受影响动作组的语义/组件/插槽策略、以及 Icon 的语义映射和资产家族均未变化时才能复用。缺失 Icon 必须独立设计，不得用页面局部替代物。
