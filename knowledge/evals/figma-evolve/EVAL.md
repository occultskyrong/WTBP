...
- 每次写入后必须对所有产品页面/状态 Frame 记录 `G-01`–`G-06`：layoutMode/布局责任层、独立右侧 annotation、文字容器边界、实例/Master 宽高比例、以及 `G-05S` 外框/产品根/系统 Chrome/TabBar/Section 归属和 `G-05V` 视觉配方、未选中固定完整截图、画布留白、上/右/下/左四边结果与产品内容隔离；未记录或未通过时不得完成。Figma 选中框、Section 边缘、画布或裁剪截图均不能作为边界证据，`G-05V` 失败时不得完成。小程序必须使用适用的小程序外框，App 必须使用声明的设备/安全区外框，Web 必须使用批准的浏览器外框或边界可见的视口容器；通用手机框或裸画布不通过。页面描述和技术说明只能出现在右侧独立 annotation 中。
