...
覆盖本地路由发现、当前会话语义判断、多候选提示、未命中处理和外部安装边界；不替代目标 Skill 的执行。语义判断只能返回已登记的建议，不能触发安装或外部副作用。只有唯一命中的受控外部路由在当前会话确认后才可显式安装，且不得提交或调用未声明的外部服务。
...
- 语义判断由当前会话完成，不启动第二个 LLM；必须返回已登记的 Skill/路由、理由和结构化缺失信息，并在不确定时澄清或回退。
