...
9. Re-read changed nodes, run the shared post-write structural gate (`G-01`–`G-06`) across every product page/state frame, inspect shell and annotation geometry, record `G-05S`/`G-05V`, and take unselected fixed full-shell screenshots with canvas margin and top/right/bottom/left edge outcomes. Identify downstream targets that need `figma-to-product` or `figma-verify` again.
...
- A write is blocked when the target shell/root ownership or dimensions are absent, the visual recipe is missing, the full unselected four-edge shell is not visible, or descriptive/technical/review text appears inside the product frame. These are `G-05S`/`G-05V`/G-06 failures and require a named approved exception.
...
- Every affected and unchanged product page/state frame has G-01 through G-06 results, including `G-05S`/`G-05V`, or a named approved exception; every new/structurally changed base batch has passed BF-01 through BF-06, including `BF-02S`/`BF-02V`, first.
