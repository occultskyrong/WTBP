...
3. Resolve project-contract conflicts. For a structural change or `regenerate`, derive or refresh `ARC-YYYYMMDD-VNN` from the approved PDC before creating or confirming `INV-YYYYMMDD-VNN`; a narrow patch may reuse the current architecture only when its contract is unchanged. Record modules, routes, shells, reuse boundaries, states/transitions, and affected base-frame batch. Obtain approval before mutation unless an already-approved inventory explicitly covers the change.
...
5. In `patch` mode, mutate only the target nodes and necessary shared components. Preserve valid Auto Layout, constraints, variants, and asset links.
