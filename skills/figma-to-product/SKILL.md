...
8. Before styling any page, implement the structural base-frame batch for every declared page/state: target shell, page root, content regions, normal-flow/Auto Layout parents, state slots, and right-side annotation sibling. Use neutral content and keep page-specific decoration out of this pass.
...
- The target, repository, project design contract (`PDC-01`–`PDC-06`), derived architecture (`ARC-YYYYMMDD-VNN`), action-group contract (`AGC-YYYYMMDD-VNN` with `AGC-01`–`AGC-06`), Icon inventory (`ICON-YYYYMMDD-VNN` with `IA-01`–`IA-07`), inventory, and applicable I-01–I-10 cases are explicit.
