...
实现前读取 [`../../knowledge/design-workflow.md`](../../knowledge/design-workflow.md)。它定义共享的简报/清单门禁、证据来源、完整页面要求、文案分类和真实夹具验收契约。
...
5. 编辑前确认清单功能 ID、终端、权限/数据范围、重要状态、建议文案标记和证据来源。实现请求改变范围或行为时，回到清单门禁，不得自行补全。
...
2. 选择所有适用的 I-01–I-07 用例，并在编辑前记录 target、节点、预期布局责任层、viewport、状态和所需失败证据。
3. 按目标项目既有约定增量实现，保护无关代码；不得提交、推送或修改其他 target。
4. 渲染完整容器页面和重要状态，不得交付裁剪组件。新增产品文案要分类，设计说明放在产品 UI 外。
5. 使用精确 Figma 资源；不得手绘替代图标或静默回退系统字体。
6. 为每个声明的 viewport/状态运行目标检查并捕获确定性运行证据。代码 diff 或单张截图不是验收；将完整夹具交给 `figma-verify`。
