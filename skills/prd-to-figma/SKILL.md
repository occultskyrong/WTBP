...
14. Add a standalone right-side annotation block for every affected frame, keep design/project/technical descriptions out of the product frame, classify every in-frame text node, and record `G-05S`/`G-05V`: shell/root bounds and ownership, visual-boundary recipe, plus an unselected fixed full-shell capture with canvas margin and top/right/bottom/left edge outcomes. After each write, run the shared post-write structural gate (`G-01`–`G-06`) across every product page/state frame, including unchanged frames. Return concrete file, page, frame, component, and state node links.
...
- Every approved page/state has passed `BF-01` through `BF-06` before styling, including `BF-02S`/`BF-02V`; every product page/state frame has G-01 through G-06 results, including `G-05S`/`G-05V`, unselected four-edge full-shell evidence, text/accessibility evidence, and transition decisions.
